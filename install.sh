...
echo "Looking for Cargo.toml files in the repository..."
CARGO_FILES=$(find "$TEMP_DIR/soundness-layer" -name "Cargo.toml" -type f)

if [ -z "$CARGO_FILES" ]; then
    echo -e "${RED}Error: Could not find any Cargo.toml files in the repository.${NC}"
    echo "Repository structure might have changed or this might not be a Rust project."
    exit 1
fi

# Display found Cargo.toml files
echo "Found these Cargo.toml files:"
echo "$CARGO_FILES"

# Try to find the main CLI directory - look for a CLI-related Cargo.toml
CLI_DIR=""
for cargo_file in $CARGO_FILES; do
    dir_name=$(dirname "$cargo_file")
    if grep -q "name.*soundness" "$cargo_file" || grep -q "name.*cli" "$cargo_file"; then
        CLI_DIR="$dir_name"
        echo -e "${GREEN}Found potential CLI directory at: $CLI_DIR${NC}"
        break
    fi
done

# If we couldn't find by name, use the first Cargo.toml's directory
if [ -z "$CLI_DIR" ]; then
    CLI_DIR=$(dirname "$(echo "$CARGO_FILES" | head -n 1)")
    echo -e "${YELLOW}Could not find a specific CLI Cargo.toml. Using: $CLI_DIR${NC}"
fi

# Navigate to the directory with Cargo.toml
cd "$CLI_DIR"
