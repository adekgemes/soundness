...
# Find the correct CLI directory
echo "Finding the CLI directory..."
CLI_DIR=""

# Option 1: Check the expected path
if [ -f "$TEMP_DIR/soundness-layer/soundness-cli/package.json" ]; then
    CLI_DIR="$TEMP_DIR/soundness-layer/soundness-cli"
# Option 2: Check if it's directly in the root
elif [ -f "$TEMP_DIR/soundness-layer/package.json" ]; then
    CLI_DIR="$TEMP_DIR/soundness-layer"
# Option 3: Search for any directory containing package.json
else
    CLI_DIR=$(find "$TEMP_DIR/soundness-layer" -name "package.json" -exec dirname {} \; | head -n 1)
fi

# Check if we found the CLI directory
if [ -z "$CLI_DIR" ]; then
    echo -e "${RED}Error: Could not find the CLI directory with package.json file.${NC}"
    echo "Repository structure might have changed. Please check the repository and update the script."
    exit 1
fi

echo "Found CLI directory at: $CLI_DIR"
